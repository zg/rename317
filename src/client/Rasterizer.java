final class Rasterizer extends DrawingArea {
	...
	public static void initTexturePools(int texturePoolSize) {
	...
	public static void setPalleteForBrightness(double brightness) {
	...
	public static void drawTriangle(int y_a, int y_b, int y_c, int x_a, int x_b, int x_c, int c_a, int c_b, int c_c) {
		...
		if (y_a <= y_b && y_a <= y_c) {
			...
			if (y_b < y_c) {
				...
				if (y_a != y_b && j3 < j2 || y_a == y_b && j3 > l2) {
					...
					for (y_a = lineOffsets[y_a]; --y_b >= 0; y_a += DrawingArea.width) {
						drawShadedLine(DrawingArea.pixels, y_a, x_c >> 16, x_a >> 16, c_c >> 7, c_a >> 7);
						x_c += j3;
						x_a += j2;
						c_c += k3;
						c_a += k2;
					...
					while (--y_c >= 0) {
						drawShadedLine(DrawingArea.pixels, y_a, x_c >> 16, x_b >> 16, c_c >> 7, c_b >> 7);
						x_c += j3;
						x_b += l2;
						c_c += k3;
						c_b += i3;
				...
				y_b -= y_a;
				for (y_a = lineOffsets[y_a]; --y_b >= 0; y_a += DrawingArea.width) {
					drawShadedLine(DrawingArea.pixels, y_a, x_a >> 16, x_c >> 16, c_a >> 7, c_c >> 7);
					x_c += j3;
					x_a += j2;
					c_c += k3;
					c_a += k2;
				...
				while (--y_c >= 0) {
					drawShadedLine(DrawingArea.pixels, y_a, x_b >> 16, x_c >> 16, c_b >> 7, c_c >> 7);
					x_c += j3;
					x_b += l2;
					c_c += k3;
					c_b += i3;
			...
			if (y_a != y_c && j3 < j2 || y_a == y_c && l2 > j2) {
				...
				for (y_a = lineOffsets[y_a]; --y_c >= 0; y_a += DrawingArea.width) {
					drawShadedLine(DrawingArea.pixels, y_a, x_b >> 16, x_a >> 16, c_b >> 7, c_a >> 7);
					x_b += j3;
					x_a += j2;
					c_b += k3;
					c_a += k2;
			...
			y_c -= y_a;
			for (y_a = lineOffsets[y_a]; --y_c >= 0; y_a += DrawingArea.width) {
				drawShadedLine(DrawingArea.pixels, y_a, x_a >> 16, x_b >> 16, c_a >> 7, c_b >> 7);
				x_b += j3;
				x_a += j2;
				c_b += k3;
				c_a += k2;
		...
		if (y_b <= y_c) {
			...
			if (y_c < y_a) {
				...
				if (y_b != y_c && j2 < l2 || y_b == y_c && j2 > j3) {
					...
					for (y_b = lineOffsets[y_b]; --y_c >= 0; y_b += DrawingArea.width) {
						drawShadedLine(DrawingArea.pixels, y_b, x_a >> 16, x_b >> 16, c_a >> 7, c_b >> 7);
						x_a += j2;
						x_b += l2;
						c_a += k2;
						c_b += i3;
					...
					while (--y_a >= 0) {
						drawShadedLine(DrawingArea.pixels, y_b, x_a >> 16, x_c >> 16, c_a >> 7, c_c >> 7);
						x_a += j2;
						x_c += j3;
						c_a += k2;
						c_c += k3;
				...
				y_c -= y_b;
				for (y_b = lineOffsets[y_b]; --y_c >= 0; y_b += DrawingArea.width) {
					drawShadedLine(DrawingArea.pixels, y_b, x_b >> 16, x_a >> 16, c_b >> 7, c_a >> 7);
					x_a += j2;
					x_b += l2;
					c_a += k2;
					c_b += i3;
				...
				while (--y_a >= 0) {
					drawShadedLine(DrawingArea.pixels, y_b, x_c >> 16, x_a >> 16, c_c >> 7, c_a >> 7);
					x_a += j2;
					x_c += j3;
					c_a += k2;
					c_c += k3;
			...
			if (j2 < l2) {
				...
				for (y_b = lineOffsets[y_b]; --y_a >= 0; y_b += DrawingArea.width) {
					drawShadedLine(DrawingArea.pixels, y_b, x_c >> 16, x_b >> 16, c_c >> 7, c_b >> 7);
					x_c += j2;
					x_b += l2;
					c_c += k2;
					c_b += i3;
			...
			y_a -= y_b;
			for (y_b = lineOffsets[y_b]; --y_a >= 0; y_b += DrawingArea.width) {
				drawShadedLine(DrawingArea.pixels, y_b, x_b >> 16, x_c >> 16, c_b >> 7, c_c >> 7);
				x_c += j2;
				x_b += l2;
				c_c += k2;
				c_b += i3;
		...
		if (y_a < y_b) {
			...
			if (l2 < j3) {
				...
				for (y_c = lineOffsets[y_c]; --y_a >= 0; y_c += DrawingArea.width) {
					drawShadedLine(DrawingArea.pixels, y_c, x_b >> 16, x_c >> 16, c_b >> 7, c_c >> 7);
					x_b += l2;
					x_c += j3;
					c_b += i3;
					c_c += k3;
				...
				while (--y_b >= 0) {
					drawShadedLine(DrawingArea.pixels, y_c, x_b >> 16, x_a >> 16, c_b >> 7, c_a >> 7);
					x_b += l2;
					x_a += j2;
					c_b += i3;
					c_a += k2;
			...
			y_a -= y_c;
			for (y_c = lineOffsets[y_c]; --y_a >= 0; y_c += DrawingArea.width) {
				drawShadedLine(DrawingArea.pixels, y_c, x_c >> 16, x_b >> 16, c_c >> 7, c_b >> 7);
				x_b += l2;
				x_c += j3;
				c_b += i3;
				c_c += k3;
			...
			while (--y_b >= 0) {
				drawShadedLine(DrawingArea.pixels, y_c, x_a >> 16, x_b >> 16, c_a >> 7, c_b >> 7);
				x_b += l2;
				x_a += j2;
				c_b += i3;
				c_a += k2;
		...
		if (l2 < j3) {
			...
			for (y_c = lineOffsets[y_c]; --y_b >= 0; y_c += DrawingArea.width) {
				drawShadedLine(DrawingArea.pixels, y_c, x_a >> 16, x_c >> 16, c_a >> 7, c_c >> 7);
				x_a += l2;
				x_c += j3;
				c_a += i3;
				c_c += k3;
		...
		y_b -= y_c;
		for (y_c = lineOffsets[y_c]; --y_b >= 0; y_c += DrawingArea.width) {
			drawShadedLine(DrawingArea.pixels, y_c, x_c >> 16, x_a >> 16, c_c >> 7, c_a >> 7);
			x_a += l2;
			x_c += j3;
			c_a += i3;
			c_c += k3;
	...
	public static void drawShadedLine(int[] pixels, int off, int start, int end, int color_index, int grad) {
		int rgb;
		int k;
		int l1 = 0;
		if (aBoolean1462) {
			if (end > DrawingArea.viewportEx)
				end = DrawingArea.viewportEx;
			if (start < 0) {
				color_index -= start * l1;
				start = 0;
			}
		}
		if (start < end) {
			off += start;
			color_index += l1 * start;
			if (aBoolean1464) {
				k = end - start >> 2;
				if (k > 0)
					l1 = (grad - color_index) * anIntArray1468[k] >> 15;
				else
					l1 = 0;
				if (alpha == 0) {
					if (k > 0) {
						do {
							rgb = HSL2RGB[color_index >> 8];
							color_index += l1;
							pixels[off++] = rgb;
							pixels[off++] = rgb;
							pixels[off++] = rgb;
					...
					if (k > 0) {
						do {
							...
							int h = pixels[off];
							pixels[off++] = (rgb + ((h & 0xff00ff) * j2 >> 8 & 0xff00ff) + ((h & 0xff00) * j2 >> 8 & 0xff00));
							h = pixels[off];
							pixels[off++] = (rgb + ((h & 0xff00ff) * j2 >> 8 & 0xff00ff) + ((h & 0xff00) * j2 >> 8 & 0xff00));
							h = pixels[off];
							pixels[off++] = (rgb + ((h & 0xff00ff) * j2 >> 8 & 0xff00ff) + ((h & 0xff00) * j2 >> 8 & 0xff00));
					...
					if (k > 0) {
						...
						rgb = (((rgb & 0xff00ff) * l2 >> 8 & 0xff00ff) + ((rgb & 0xff00) * l2 >> 8 & 0xff00));
						do {
							int i_61_ = pixels[off];
							pixels[off++] = (rgb + ((i_61_ & 0xff00ff) * j2 >> 8 & 0xff00ff) + ((i_61_ & 0xff00) * j2 >> 8 & 0xff00));
						} while (--k > 0);
					}
				}
			} else {
				int i2 = (grad - color_index) / (end - start);
				k = end - start;
				if (alpha == 0) {
					do {
						pixels[off++] = HSL2RGB[color_index >> 8];
						color_index += i2;
					} while (--k > 0);
				} else {
					int i4 = alpha;
					...
					do {
						rgb = HSL2RGB[color_index >> 8];
						color_index += i2;
						rgb = (((rgb & 0xff00ff) * j4 >> 8 & 0xff00ff) + ((rgb & 0xff00) * j4 >> 8 & 0xff00));
						int i_64_ = pixels[off];
						pixels[off++] = (rgb + ((i_64_ & 0xff00ff) * i4 >> 8 & 0xff00ff) + ((i_64_ & 0xff00) * i4 >> 8 & 0xff00));
					} while (--k > 0);
				}
			}
		}
	...
	private static void draw2DJagColouredShape_(int ai[], int i, int l, int i1, int jgx, int k1) {
		...
		if (aBoolean1464) {
			...
			if (aBoolean1462) {
				if (i1 - l > 3)
					...
					l1 = 0;
				if (i1 > DrawingArea.viewportEx)
					i1 = DrawingArea.viewportEx;
				if (l < 0) {
					jgx -= l * l1;
					l = 0;
				}
