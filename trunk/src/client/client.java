...
public class client extends RSApplet {
	...
	private void updateWorldObjects() {
		...
		System.gc();
		Rasterizer.initTexturePools(20);
	...
	private void applyConfigChange(int i) {
		...
		if (j == 1) {
			if (k == 1)
				Rasterizer.setPalleteForBrightness(0.90000000000000002D);
			if (k == 2)
				Rasterizer.setPalleteForBrightness(0.80000000000000004D);
			if (k == 3)
				Rasterizer.setPalleteForBrightness(0.69999999999999996D);
			if (k == 4)
				Rasterizer.setPalleteForBrightness(0.59999999999999998D);
			ItemDef.mruNodes1.unlinkAll();
		...
		for (int j = 0; j < sessionNpcsAwaitingUpdatePtr; j++) {
			...
			if ((l & 0x10) != 0) {
				...
				if (i1 == npc.animation && i1 != -1) {
					...
					if (l2 == 2)
						npc.anInt1530 = 0;
				} else if (i1 == -1 || npc.animation == -1 || Animation.anims[i1].anInt359 >= Animation.anims[npc.animation].anInt359) {
					npc.animation = i1;
					npc.anInt1527 = 0;
					npc.anInt1528 = 0;
					npc.anInt1529 = i2;
	...
	private void entityUpdateBlock(Entity entity)// realy?
	{
		...
		else if (entity.anInt1548 >= currentTime)
	...
	private void appendPlayerUpdateMask(int i, int j, Packet stream, Player player) {
		...
		if ((i & 8) != 0) {
			...
			if (l == player.animation && l != -1) {
				...
				if (i3 == 2)
					player.anInt1530 = 0;
			} else if (l == -1 || player.animation == -1 || Animation.anims[l].anInt359 >= Animation.anims[player.animation].anInt359) {
				player.animation = l;
				player.anInt1527 = 0;
				player.anInt1528 = 0;
				player.anInt1529 = i2;
