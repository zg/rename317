...
final class MapRegion {
	...
	public final void addTiles(TileSetting aclass11[], SceneGraph sceneGraph) {
		...
		for (int z = 0; z < 4; z++) {
			...
			for (int Y = 1; Y < yMapSize - 1; Y++) {
				for (int X = 1; X < xMapSize - 1; X++) { // This is object shadows
				...
				for (int __y = 0; __y <= yMapSize; __y++) {
					for (int __x = 0; __x <= xMapSize; __x++) {
						if ((tile_culling_bitmap[__z][__x][__y] & x_flag) != 0) {
							...
							for_lowest_z_flagged: for (; lowest_z_flagged > 0; lowest_z_flagged--) {
							...
							for_highest_z_flagged: for (; highest_z_flagged < _z; highest_z_flagged++) {
								for (int ___y = lowest_y_flagged; ___y <= highest_y_flagged; ___y++)
	...
	private void addObjectToRenderer(int i, SceneGraph sceneGraph, TileSetting tileSetting, int j, int k, int l, int i1, int j1) {
	...
	public final void method179(int i, int j, TileSetting aclass11[], int l, int i1, byte abyte0[], int j1, int k1, int l1) {
	...
	private void readTile(int i, int j, Packet stream, int k, int l, int i1, int k1) {
	...
	public final void method183(TileSetting aclass11[], SceneGraph sceneGraph, int i, int j, int k, int l, byte abyte0[], int i1, int j1, int k1) {
		label0: {
			Packet stream = new Packet(abyte0);
			int l1 = -1;
			do {
	...
	private static final int faceXOffset[] = { 1, 0, -1, 0 };
	...
	private static final int faceYOffset[] = { 0, -1, 0, 1 };
	static int setZ = 99;
	...
	private static final int bitValues[] = { 1, 2, 4, 8 };
}
