...
public final class RgbImage extends DrawingArea {
	...
	public void rotate(int height, int angle, int widthmap[], int hingesize, int ai1[], int center_y, int j1, int k1, int width, int center_x) {
		try {
			int j2 = -width / 2;
			int k2 = -height / 2;
	...
	public void drawSpriteWithDimens(int xPos, int yPos, int vertSize) {
		...
		if (!(horizSize <= 0 || vertSize <= 0)) {
			method349(DrawingArea.pixels, myPixels, i1, l, horizSize, vertSize, l1, i2);
		}
	}
	public int myPixels[];
	public int myWidth;
